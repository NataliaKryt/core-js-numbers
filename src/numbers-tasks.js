...
function getFibonacciNumber(index) {
  return Math.round((1.618034 ** index - (-0.618034) ** index) / 5 ** 0.5);
}
...
function isPowerOfTwo(num) {
  return 2 ** Math.round(Math.log(num) / Math.log(2)) === num;
}
...
function getCountOfOddNumbers(number) {
  if (number % 2 === 1) {
    return Math.round((Math.abs(number) + 1) / 2);
  }
